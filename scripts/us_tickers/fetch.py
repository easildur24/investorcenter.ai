...
def _download_file(url: str, session: Optional[requests.Session] = None) -> str:
    ...
    try:
        ...
        logger.info(f"Successfully downloaded {url} ({len(content)} characters)")
        return content
...
def _filter_and_clean_data(
    ...
    logger.info(f"Final result: {len(df)} unique tickers (from {initial_count} total rows)")
    return df
...
def get_exchange_listed_tickers(
    ...
    try:
        exchange_config = validate_exchange_config(exchanges, include_etfs, include_test_issues)
    ...
    logger.info(f"Include ETFs: {include_etfs}, Include test issues: {include_test_issues}")
    
    # Check cache first if caching is enabled
    if cache_ttl_hours and cache_ttl_hours > 0:
        cache = SimpleCache(config.cache_dir)
        cache_key = f"tickers_{','.join(exchanges)}_{include_etfs}_{include_test_issues}"
    ...
    if nasdaq_df.empty and other_df.empty:
        raise RuntimeError("Failed to fetch data from all sources. Please check your internet connection and try again.")
    ...
    if nasdaq_df.empty:
        ...
        logger.info(f"Combined {len(nasdaq_df)} Nasdaq + {len(other_df)} other = {len(combined_df)} total rows")
