...
class Config:
    ...
    def nasdaq_listed_url(self) -> str:
        ...
        return f"{self.nasdaq_base_url.rstrip('/')}/{self.nasdaq_listed_filename}"
    
    @property
    def other_listed_url(self) -> str:
        """Get full other listed URL."""
        return f"{self.nasdaq_base_url.rstrip('/')}/{self.other_listed_filename}"
    
    @classmethod
    def from_env(cls) -> "Config":
        """Create configuration from environment variables."""
        return cls(
            nasdaq_base_url=os.getenv("NASDAQ_BASE_URL", "https://www.nasdaqtrader.com/dynamic/symdir/"),
            timeout_seconds=int(os.getenv("TIMEOUT_SECONDS", "20")),
            max_retries=int(os.getenv("MAX_RETRIES", "3")),
            backoff_factor=float(os.getenv("BACKOFF_FACTOR", "1.0")),
            cache_ttl_hours=int(os.getenv("CACHE_TTL_HOURS", "24")),
            cache_dir=os.getenv("CACHE_DIR", ".cache"),
            default_include_etfs=os.getenv("DEFAULT_INCLUDE_ETFS", "false").lower() == "true",
            default_include_test_issues=os.getenv("DEFAULT_INCLUDE_TEST_ISSUES", "false").lower() == "true",
        )
