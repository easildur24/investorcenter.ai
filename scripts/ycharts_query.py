...
try:
    ...
    print(
        "Warning: pandas not available. Install with 'pip install pandas' for better data formatting."
    )
...
class YChartsAPIClient:
    ...
    def get_company_latest_data(
        ...
        try:
            self.logger.info(
                f"Fetching latest data for symbols: {symbols}, metrics: {metrics}"
            )
    ...
    def get_company_historical_data(
        ...
        try:
            self.logger.info(
                f"Fetching historical data for symbols: {symbols}, metrics: {metrics}"
            )
            self.logger.info(
    ...
    def get_company_info(
        self, symbols: List[str], info_fields: List[str]
    ) -> Dict[str, Any]:
        """
        ...
        try:
            self.logger.info(
                f"Fetching company info for symbols: {symbols}, fields: {info_fields}"
            )
    ...
    def get_mutual_fund_data(
        ...
        try:
            self.logger.info(
                f"Fetching mutual fund data for symbols: {symbols}, metrics: {metrics}"
            )
    ...
    def format_response(
        self, response: Dict[str, Any], output_format: str = "json"
    ) -> str:
        """
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Query YCharts API for financial data"
    )
    parser.add_argument(
        "--api-key",
        type=str,
        help="YCharts API key (or set YCHARTS_API_KEY env var)",
    )
    parser.add_argument(
        "--symbols",
        type=str,
    ...
    parser.add_argument(
        "--query-type",
        type=str,
        choices=["latest", "historical", "info", "mutual_fund", "indicator"],
        default="latest",
    ...
    if not api_key:
        print(
            "Error: API key required. Use --api-key argument or set YCHARTS_API_KEY environment variable."
        )
        sys.exit(1)
    ...
    if args.query_type == "latest":
        ...
        response = client.get_company_historical_data(
            args.symbols, args.metrics, start_date, end_date
        )
        ...
        response = client.get_indicator_data(
