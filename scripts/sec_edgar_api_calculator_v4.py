#!/usr/bin/env python3
"""
SEC EDGAR API Calculator v4 - EXACT MATCHES
Using the proven method: Calculate missing Q4 from annual data for EXACT results
"""
...
class SECEdgarCalculatorV4:
    ...
    def get_exact_ttm_revenue(self, facts_data):
        """Get EXACT TTM revenue using the proven Tesla method"""
        try:
            ...
            # Find which concept has the most recent data
            ...
            if best_concept:
                ...
                # Use the best concept data
                usd_data = us_gaap[best_concept].get('units', {}).get('USD', [])
                
                # Get quarterly data (with frames) - include recent data
                quarterly_data = [item for item in usd_data 
                                if item.get('frame') and 'Q' in item.get('frame', '')
                                and item.get('end', '') >= '2022-01-01']  # Filter for recent data only
                
                print(f"  🔍 Debug: Found {len(quarterly_data)} quarterly data points for {best_concept}")
                    ...
                    if len(quarterly_data) >= 4:
                        ...
                        if len(sorted_quarters) >= 4:
                            ...
                            if months_diff <= 15:  # All 4 quarters available
                                ...
                                for i, q in enumerate(sorted_quarters[:4]):
                                    print(f"  Q{i+1}: {q.get('end')} = ${q.get('val', 0)/1000000000:.2f}B")
                                print(f"  TTM TOTAL: ${ttm_revenue/1000000000:.2f}B")
                                
                                return ttm_revenue
                            
                        # If we reach here, we need to calculate missing Q4 from annual data
                        if annual_data:
                            sorted_annual = sorted(annual_data, key=lambda x: x.get('end', ''), reverse=True)
                            
                            # Get the most recent 3 quarters
                            q1 = sorted_quarters[0].get('val', 0)  # Most recent
                            q2 = sorted_quarters[1].get('val', 0)  # 2nd most recent
                            q3 = sorted_quarters[2].get('val', 0)  # 3rd most recent
                            
                            # Get the annual data that contains the missing Q4
                            ...
                            # Find which quarters are in this annual period
                            ...
                            if len(quarters_in_annual) == 3:  # Missing Q4
                                ...
                                print(f"  Q1 (most recent): ${q1/1000000000:.2f}B")
    ...
    def calculate_exact_metrics(self, symbol):
        """Calculate EXACT metrics using proven method"""
...
def main():
    ...
    if metrics:
        print(f"✅ Successfully calculated exact metrics for {symbol}")
        ...
        if 'revenue_ttm' in metrics:
            ...
            if symbol == 'TSLA':
                ...
                if diff < 0.1:
                    ...
                    print(f"❌ Still off by ${diff:.2f}B (vs ${target:.2f}B target)")
    else:
