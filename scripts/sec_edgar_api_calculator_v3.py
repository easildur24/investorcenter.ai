...
class SECEdgarCalculatorV3:
    ...
    def calculate_and_store(self, symbol):
        ...
        # Capital Expenditures - comprehensive approach for different companies
        capex_concepts = [
            'PaymentsToAcquirePropertyPlantAndEquipment',
            'PaymentsToAcquireProductiveAssets',  # NVIDIA uses this - closer to reference
            'CapitalExpenditures',
        ...
        # Try all concepts and use the one that makes most sense
        for concept in capex_concepts:
            capex_value = self.extract_cash_flow_ttm(facts_data, concept, f'CapEx ({concept})')
            if capex_value and abs(capex_value) > best_capex_value:
                best_capex_value = abs(capex_value)
                metrics['capital_expenditures_ttm'] = abs(capex_value)  # CapEx is always positive
                print(f"  📊 Best CapEx concept: {concept}")
        
        if not metrics['capital_expenditures_ttm']:
            # Fallback to quarterly sum approach
            for concept in capex_concepts:
                capex_value = self.extract_ttm_simple(facts_data, [concept], f'CapEx Fallback ({concept})')
                if capex_value:
                    metrics['capital_expenditures_ttm'] = abs(capex_value)
                    break
