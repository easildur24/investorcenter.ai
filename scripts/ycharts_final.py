...
class YChartsClient:
    ...
    def __init__(self, api_key=None):
        ...
        if not self.api_key:
            raise ValueError(
                "API key required. Set YCHARTS_API_KEY in .env file."
            )
    ...
    def get_series_data(self, security_id, metric_ids):
        ...
        params = {"security_id": security_id, "metric_ids": metric_ids}
    ...
    def _make_request(self, endpoint, params=None):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                return {
                    "error": "Forbidden - check account permissions or contact YCharts support"
                }
...
def main():
    ...
    parser.add_argument(
        "--symbols", nargs="+", default=["AAPL"], help="Symbols to query"
    )
    parser.add_argument(
        "--metrics",
        nargs="+",
        default=["1day_return"],
        help="Metrics to fetch",
    )
    parser.add_argument(
        "--test", action="store_true", help="Test connection only"
    )
