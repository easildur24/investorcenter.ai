...
// Uses the original (non-enriched) query for backward compatibility.
...
// GetWatchListWithEnrichedItems retrieves a watch list with enriched items
// (IC Score, fundamentals, Reddit data, alert counts) and real-time prices.
func (s *WatchListService) GetWatchListWithEnrichedItems(watchListID string, userID string) (*models.WatchListWithItemsEnriched, error) {
	// Get watch list metadata
	watchList, err := database.GetWatchListByID(watchListID, userID)
	if err != nil {
		return nil, err
