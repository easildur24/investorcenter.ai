...
function SectionHeader({ title }: { title: string }) {
  return (
    <h4 className="text-sm font-semibold text-ic-text-secondary uppercase tracking-wide mb-3 mt-1">
      {title}
    </h4>
  );
}
...
function GroupCard({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="bg-ic-bg-secondary rounded-lg p-4">
      <h3 className="text-base font-semibold text-ic-text-primary mb-4 pb-2 border-b border-ic-border">
        {title}
      </h3>
      {children}
    </div>
      ...
      <div className="p-6">
        <div className="text-center py-12">
          ...
          <p className="text-ic-text-dim text-sm mt-2">
            Key stats can be ingested via the API for this ticker.
          </p>
        </div>
      </div>
    );
    ...
    <div className="p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <GroupCard title="Income Statement">
          ...
          <MetricRow
            label="Revenue QoQ"
            value={formatPct(income.revenue_quarterly_yoy_growth)}
            className={valueColor(income.revenue_quarterly_yoy_growth)}
          />
          <MetricRow
            label="EBITDA QoQ"
            value={formatPct(income.ebitda_quarterly_yoy_growth)}
            className={valueColor(income.ebitda_quarterly_yoy_growth)}
          />
          <MetricRow
            label="EPS Diluted QoQ"
            value={formatPct(income.eps_diluted_quarterly_yoy_growth)}
            className={valueColor(income.eps_diluted_quarterly_yoy_growth)}
          />
        </GroupCard>
        ...
        <GroupCard title="Balance Sheet">
          ...
          <MetricRow
            ...
            value={formatCurrency(balance.total_liabilities_quarterly)}
          />
          <MetricRow
            label="Shareholders' Equity"
            value={formatCurrency(balance.shareholders_equity_quarterly)}
          />
          <MetricRow label="Book Value" value={formatCurrency(balance.book_value_quarterly)} />
          <MetricRow
            label="Cash & Short-Term Inv."
            value={formatCurrency(balance.cash_and_short_term_investments_quarterly)}
          />
          <MetricRow
            label="Long-Term Assets"
            value={formatCurrency(balance.total_long_term_assets_quarterly)}
          />
          <MetricRow
            label="Long-Term Debt"
            value={formatCurrency(balance.total_long_term_debt_quarterly)}
          />
        </GroupCard>
        ...
        <GroupCard title="Cash Flow">
          <MetricRow
            ...
            value={formatCurrency(cashFlow.cash_from_operations_ttm)}
          />
          <MetricRow
            label="Investing Cash Flow TTM"
            value={formatCurrency(cashFlow.cash_from_investing_ttm)}
          />
          <MetricRow
            label="Financing Cash Flow TTM"
            value={formatCurrency(cashFlow.cash_from_financing_ttm)}
          />
          ...
          <MetricRow
            ...
            className={valueColor(cashFlow.free_cash_flow)}
          />
          <MetricRow
            label="Changes in Working Capital"
            value={formatCurrency(cashFlow.changes_in_working_capital_ttm)}
          />
        </GroupCard>
        ...
        <GroupCard title="Profitability & Returns">
          ...
          <MetricRow
            ...
            value={formatNumber(commonSize.shares_outstanding)}
          />
        </GroupCard>
        ...
        <GroupCard title="Analyst Estimates">
          ...
          <MetricRow
            ...
            value={formatPrice(estimates.eps_estimate_current_quarter)}
          />
          <MetricRow
            label="Next Quarter"
            value={formatPrice(estimates.eps_estimate_next_quarter)}
          />
          <MetricRow
            label="Current Fiscal Year"
            value={formatPrice(estimates.eps_estimate_current_fiscal_year)}
          />
          <MetricRow
            label="Next Fiscal Year"
            value={formatPrice(estimates.eps_estimate_next_fiscal_year)}
          />
          <SectionHeader title="Revenue Estimates" />
          <MetricRow
            label="Current Quarter"
            value={formatCurrency(estimates.revenue_estimate_current_quarter)}
          />
          <MetricRow
            label="Next Quarter"
            value={formatCurrency(estimates.revenue_estimate_next_quarter)}
          />
          <MetricRow
            label="Current Fiscal Year"
            value={formatCurrency(estimates.revenue_estimate_current_fiscal_year)}
          />
          <MetricRow
            label="Next Fiscal Year"
            value={formatCurrency(estimates.revenue_estimate_next_fiscal_year)}
          />
        </GroupCard>
        ...
        <GroupCard title="Stock Price Performance">
          <MetricRow
            label="1 Month"
            value={formatPct(stockPerf['1_month_total_returns'])}
            className={valueColor(stockPerf['1_month_total_returns'])}
          />
          <MetricRow
            label="3 Months"
            value={formatPct(stockPerf['3_month_total_returns'])}
            className={valueColor(stockPerf['3_month_total_returns'])}
          />
          <MetricRow
            label="6 Months"
            value={formatPct(stockPerf['6_month_total_returns'])}
            className={valueColor(stockPerf['6_month_total_returns'])}
          />
          <MetricRow
            label="YTD"
            value={formatPct(stockPerf.ytd_total_returns)}
            className={valueColor(stockPerf.ytd_total_returns)}
          />
          <MetricRow
            label="1 Year"
            value={formatPct(stockPerf['1_year_total_returns'])}
            className={valueColor(stockPerf['1_year_total_returns'])}
          />
          <MetricRow
            label="3 Year (Ann.)"
            value={formatPct(stockPerf['3_year_annualized_returns'])}
            className={valueColor(stockPerf['3_year_annualized_returns'])}
          />
          <MetricRow
            label="5 Year (Ann.)"
            value={formatPct(stockPerf['5_year_annualized_returns'])}
            className={valueColor(stockPerf['5_year_annualized_returns'])}
          />
        ...
        <GroupCard title="Risk Metrics">
          ...
          <MetricRow
            label="Alpha (5Y)"
            value={formatDec(risk.alpha_5y)}
            className={valueColor(risk.alpha_5y)}
          />
          <MetricRow
            label="Annualized Std Dev (5Y)"
            value={formatPct(risk.annualized_std_dev_5y)}
          />
          <MetricRow
            label="Sharpe Ratio (5Y)"
            value={formatDec(risk.historical_sharpe_ratio_5y, 4)}
          />
          <MetricRow label="Sortino Ratio (5Y)" value={formatDec(risk.historical_sortino_5y, 4)} />
          <MetricRow
            label="Max Drawdown (5Y)"
            value={formatPct(risk.max_drawdown_5y)}
            className="text-ic-negative"
          />
        ...
        <GroupCard title="Liquidity & Solvency">
          ...
          <MetricRow
            ...
            value={formatCurrency(liquidity.free_cash_flow_quarterly)}
            className={valueColor(liquidity.free_cash_flow_quarterly)}
          />
        </GroupCard>
        ...
        <GroupCard title="Management Effectiveness">
          ...
          <MetricRow
            ...
            value={formatDec(mgmt.days_sales_outstanding_quarterly)}
          />
          <MetricRow
            label="Days Inventory Outstanding"
            value={formatDec(mgmt.days_inventory_outstanding_quarterly)}
          />
          <MetricRow
            label="Days Payable Outstanding"
            value={formatDec(mgmt.days_payable_outstanding_quarterly)}
          />
          <MetricRow
            label="Total Receivables (Q)"
            value={formatCurrency(mgmt.total_receivables_quarterly)}
          />
        </GroupCard>
        ...
        <GroupCard title="Advanced Metrics">
          <MetricRow
            ...
            value={
              advanced.piotroski_f_score_ttm != null ? `${advanced.piotroski_f_score_ttm}/9` : 'N/A'
            }
          />
          <MetricRow label="Tobin's Q" value={formatDec(advanced.tobins_q_quarterly)} />
          <MetricRow
            label="Sustainable Growth Rate"
            value={formatPct(advanced.sustainable_growth_rate_ttm)}
          />
          <MetricRow
            label="Quality Ratio Score"
            value={
              advanced.quality_ratio_score != null ? `${advanced.quality_ratio_score}/10` : 'N/A'
            }
          />
          <MetricRow
            label="Momentum Score"
            value={advanced.momentum_score != null ? `${advanced.momentum_score}/10` : 'N/A'}
          />
          <MetricRow
            label="Market Cap Score"
            value={advanced.market_cap_score != null ? `${advanced.market_cap_score}/10` : 'N/A'}
          />
        </GroupCard>
        ...
        <GroupCard title="Dividends & Shares">
          ...
          <MetricRow
            ...
            value={
              dividends.dividend_yield_forward != null
                ? formatPct(dividends.dividend_yield_forward)
                : 'N/A'
            }
          />
          ...
          <MetricRow
            label="Last Dividend"
            value={
              dividends.last_dividend_amount != null
                ? formatPrice(dividends.last_dividend_amount)
                : 'N/A'
            }
          />
          <MetricRow
            label="Last Ex-Dividend Date"
            value={dividends.last_ex_dividend_date || 'N/A'}
          />
        </GroupCard>
        ...
        {employees.total_employees_annual && (
          <GroupCard title="Employee Metrics">
            <MetricRow
              ...
              value={formatNumber(employees.total_employees_annual)}
            />
            <MetricRow
              label="Revenue per Employee"
              value={formatCurrency(employees.revenue_per_employee_annual)}
            />
            <MetricRow
              label="Net Income per Employee"
              value={formatCurrency(employees.net_income_per_employee_annual)}
            />
          </GroupCard>
        )}
      </div>
