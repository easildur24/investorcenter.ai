...
// PostAnalystEstimates handles POST /ingest/ycharts/analyst_estimates/:ticker
// Ingests a point-in-time snapshot of analyst estimates.
// S3 key: ycharts/analyst_estimates/{TICKER}/{YYYY-MM-DD}/{timestamp}.json
func PostAnalystEstimates(c *gin.Context) {
	...
	schemaPath := "schemas/ycharts/analyst_estimates.json"
	schemaLoader := gojsonschema.NewReferenceLoader("file://" + schemaPath)
	documentLoader := gojsonschema.NewGoLoader(requestData)

	result, err := gojsonschema.Validate(schemaLoader, documentLoader)
	if err != nil {
		log.Printf("Schema validation error for analyst_estimates: %v", err)
	...
	// Generate S3 key: ycharts/analyst_estimates/{TICKER}/{YYYY-MM-DD}/{timestamp}.json
	now := time.Now().UTC()
	timestampPart := now.Format("20060102T150405Z")
	s3Key := fmt.Sprintf("ycharts/analyst_estimates/%s/%s/%s.json", ticker, asOfDate, timestampPart)
	...
	log.Printf("YCharts Analyst Estimates ingestion success: id=%d ticker=%s key=%s size=%d",
