...
// PostPerformance handles POST /ingest/ycharts/performance/:ticker
// Ingests a point-in-time snapshot of performance metrics.
// S3 key: ycharts/performance/{TICKER}/{YYYY-MM-DD}/{timestamp}.json
func PostPerformance(c *gin.Context) {
	...
	schemaPath := "schemas/ycharts/performance.json"
	schemaLoader := gojsonschema.NewReferenceLoader("file://" + schemaPath)
	documentLoader := gojsonschema.NewGoLoader(requestData)

	result, err := gojsonschema.Validate(schemaLoader, documentLoader)
	if err != nil {
		log.Printf("Schema validation error for performance: %v", err)
	...
	// Generate S3 key: ycharts/performance/{TICKER}/{YYYY-MM-DD}/{timestamp}.json
	now := time.Now().UTC()
	timestampPart := now.Format("20060102T150405Z")
	s3Key := fmt.Sprintf("ycharts/performance/%s/%s/%s.json", ticker, asOfDate, timestampPart)
	...
	log.Printf("YCharts Performance ingestion success: id=%d ticker=%s key=%s size=%d",
