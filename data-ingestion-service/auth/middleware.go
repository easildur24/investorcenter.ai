...
// WorkerMiddleware checks that the user is a worker by querying the database.
// Must be used AFTER AuthMiddleware.
func WorkerMiddleware() gin.HandlerFunc {
	return func(c *gin.Context) {
		// For the ingestion service, we rely on JWT auth only.
		// The worker check is done at the handler level if needed.
		c.Next()
	}
}
