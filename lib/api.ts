...
class ApiClient {
  ...
  async nlpScreenerQuery(
    query: string
  ): Promise<{ params: Partial<ScreenerApiParams>; explanation: string }> {
    const res = await this.request<{ params: Partial<ScreenerApiParams>; explanation: string }>(
      '/screener/nlp',
      {
        method: 'POST',
